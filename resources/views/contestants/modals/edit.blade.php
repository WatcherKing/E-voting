<div class="modal modal-right large fade" id="editContestant{{ $contestant->id }}" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Edit Contestant: {{ $contestant->name }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="{{ route('contestant.update', $contestant->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
            <div class="form-floating mb-3">
              <input type="text" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name"
                placeholder="Name" value="{{ $contestant->name }}" required>
              <label>Name</label>
            </div>
          ...
          <div class="form-group{{ $errors->has('age') ? ' has-danger' : '' }}">
            <div class="form-floating mb-3">
              <input type="number" class="form-control{{ $errors->has('age') ? ' is-invalid' : '' }}" name="age"
                placeholder="Age" value="{{ $contestant->age }}" required>
              <label>Age</label>
            </div>
          ...
          <div class="form-group input-group mb-3{{ $errors->has('image') ? ' has-danger' : '' }}">
            ...
            <input type="file" class="form-control{{ $errors->has('image') ? ' is-invalid' : '' }}"
              id="inputGroupFile01" value="{{ $contestant->image }}" name="image">
          ...
          <div class="form-group{{ $errors->has('party') ? ' has-danger' : '' }}">
            <div class="form-floating mb-3">
              <input type="text" class="form-control{{ $errors->has('party') ? ' is-invalid' : '' }}" name="party"
                placeholder="Party" value="{{ $contestant->party }}" required>
              <label>Party</label>
            </div>
          ...
          <div class="form-group mb-3" {{ $errors->has('state_id') ? ' has-danger' : '' }}>
            ...
            <select class="form-states{{ $contestant->id }}{{ $errors->has('state_id') ? ' is-invalid' : '' }}" name="state_id">
          ...
          <div class="form-group mb-3" {{ $errors->has('local_gov_id') ? ' has-danger' : '' }}>
            ...
            <select class="form-selected{{ $contestant->id }}{{ $errors->has('local_gov_id') ? ' is-invalid' : '' }}" name="local_gov_id">
            </select>
          ...
          <div class="form-group mb-3" {{ $errors->has('position_id') ? ' has-danger' : '' }}>
            ...
            <select class="form-position{{ $contestant->id }}{{ $errors->has('position_id') ? ' is-invalid' : '' }}" name="position_id">
...
@push('js')
  <script>
    new SlimSelect({
      select: '.form-states{{ $contestant->id }}',

    })
    new SlimSelect({
      select: '.form-position{{ $contestant->id }}',

    })
    $('.form-states{{ $contestant->id }}').change(function() {
      ...
      if (stateID) {
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            new SlimSelect({
              select: '.form-selected{{ $contestant->id }}',
              data: localGovs
            });
          }
        });
